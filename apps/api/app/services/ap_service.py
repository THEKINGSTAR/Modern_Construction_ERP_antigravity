...
from app.models.ap_ar import APInvoice, APInvoiceLine, Payment, PaymentAllocation, InvoiceStatus, PaymentType, PaymentStatus
from app.models.accounting import Account, AccountType
from app.schemas.ap_ar import APInvoiceCreate, PaymentCreate
...
class APService:
    ...
    def post_invoice(self, invoice_id: UUID) -> APInvoice:
        invoice = self.db.query(APInvoice).filter(APInvoice.id == invoice_id, APInvoice.tenant_id == self.tenant_id).first()
        if not invoice:
            raise HTTPException(status_code=404, detail="Invoice not found")
        if invoice.status != InvoiceStatus.DRAFT and invoice.status != InvoiceStatus.APPROVED:
    ...
    def create_payment(self, schema: PaymentCreate) -> Payment:
        payment = Payment(
            ...
            status=PaymentStatus.POSTED, # Assume payments are posted immediately
            created_by=self.current_user_id,
            updated_by=self.current_user_id
        )
        self.db.add(payment)
        ...
        ap_account_id = self._get_account(AccountType.LIABILITY)
        cash_account_id = self._get_account(AccountType.ASSET) # Assuming asset for bank/cash

        # Create GL Entry for payment
        # Debit AP Liability, Credit Cash
        journal_lines = [
            JournalLineCreate(account_id=ap_account_id, debit=schema.amount, description=f"Payment {schema.reference}"),
            JournalLineCreate(account_id=cash_account_id, credit=schema.amount, description=f"Payment {schema.reference}")
        ]
        ...
        for alloc_schema in schema.allocations:
            ...
            if alloc_schema.ap_invoice_id:
                ...
                inv = self.db.query(APInvoice).get(alloc_schema.ap_invoice_id)
                if bal <= 0:
